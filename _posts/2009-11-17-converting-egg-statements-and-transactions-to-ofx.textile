...
:textile
  ...
  Where we pipe the content of the downloaded statement into a single line ruby script that constructs the key/value pair that we need to send to the service and pipes that into curl that actually makes the POST request (the magic <code>-d@-</code> means, read from stdin).
